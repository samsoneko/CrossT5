...
def inference(signal, checkpoint_name):
    ...
    for input in train_dataloader:
        ...
        save_text(lang_result, input["L_filenames"][0], "inference/" + checkpoint_name.lstrip('/').rstrip('.tar') + "/" + signal + "/") # save the predicted descriptions
        ...
        save_latent(act_result.unsqueeze(0), input["B_filenames"][0], "inference/" + checkpoint_name.lstrip('/').rstrip('.tar') + "/" + signal + "/")
    ...
    nrmse_train, nrmse_test = evaluate(signal, appriori_len, checkpoint_name)
    ...
    if write_to_file:
        ...
        write_file = open(write_directory + "/" + file_name + ".txt", 'a')
...
def main(checkpoint):
    inference('translate', checkpoint)
    inference('describe', checkpoint)
    inference('execute', checkpoint)
    inference('repeat language', checkpoint)
    inference('repeat action', checkpoint)
