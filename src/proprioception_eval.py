...
def evaluate(signal='describe', app_length=True, checkpoint_name='cross_t5.tar'):
    ...
    predict_train, _, predtrain_bin, predtrain_len, _ = read_sequential_target('../train/inference/' + checkpoint_name.lstrip('/').rstrip('.tar') + "/" + signal + '/prediction/behavior_train/', True)
    predict_test, _, predtest_bin, predtest_len, _ = read_sequential_target('../train/inference/' + checkpoint_name.lstrip('/').rstrip('.tar') + "/" + signal + '/prediction/behavior_test/', True)
